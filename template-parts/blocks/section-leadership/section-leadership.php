...
<?php elseif ( !empty( $section_title ) && $section_title ) : ?>
	<section id="<?= esc_attr( $id ); ?>" class="<?= esc_attr( $class_name ); ?>">
		<?php if ( $block_style === 'v1' ) : ?>
			<?php get_template_part( 'template-parts/blocks/section-leadership/variants/section-leadership-v1', '', ['block_id' => $block['id']] ); ?>
		<?php elseif ( $block_style === 'v2' ) : ?>
			<?php get_template_part( 'template-parts/blocks/section-leadership/variants/section-leadership-v2', '', ['block_id' => $block['id']] ); ?>
		<?php endif; ?>
	</section>
<?php endif; ?>