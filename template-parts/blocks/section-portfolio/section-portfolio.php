<?php
/**
 * Section Portfolio Block Template.
 *
...
$block_style = get_field( 'portfolio_section_choose_variants' );
...
$class_name = 'section section-portfolio';
...
$class_name .= ' section-portfolio--style-' . $block_style;
...
<?php elseif ( !empty( $section_title ) && $section_title ) : ?>
	<section id="<?= esc_attr( $id ); ?>" data-id="<?= esc_attr( $block['id'] ); ?>" class="<?= esc_attr( $class_name ); ?>">
		<?php if ( $block_style === 'v1' ) : ?>
			<?php get_template_part( 'template-parts/blocks/section-portfolio/variants/section-portfolio-v1', '', ['block_id' => $block['id']] ); ?>
		<?php elseif ( $block_style === 'v2' ) : ?>
			<?php get_template_part( 'template-parts/blocks/section-portfolio/variants/section-portfolio-v2', '', ['block_id' => $block['id']] ); ?>
		<?php endif; ?>
	</section>
<?php endif; ?>