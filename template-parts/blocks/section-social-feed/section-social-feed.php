...
$block_style = get_field( 'social_feed_section_choose_variants' );
...
$class_name = 'section section-social-feed';
...
$class_name .= ' section-social-feed--style-' . $block_style;
...
if ( $post_type == 'manually' ) {
	...
	if ( !$feed ) {
		$is_content_show = false;
	}
}
?>

<?php if ( $is_content_show ) : ?>
	...
	<?php else : ?>
		<section id="<?= esc_attr( $id ); ?>" class="<?= esc_attr( $class_name ); ?>">
			<?php if ( $block_style == 'v1' ) : ?>
				<?php get_template_part( 'template-parts/blocks/section-social-feed/variants/section-social-feed-v1', '', ['block_id' => $block['id']] ); ?>
			<?php elseif ( $block_style == 'v2' ) : ?>
				<?php get_template_part( 'template-parts/blocks/section-social-feed/variants/section-social-feed-v2', '', ['block_id' => $block['id']] ); ?>
			<?php endif; ?>
		</section>
	<?php endif; ?>
<?php endif; ?>
