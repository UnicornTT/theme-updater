...
$block_style = get_field( 'contact_section_choose-variants' );
...
<?php elseif ( !empty( $section_title ) && $section_title ) : ?>
	<section id="<?= esc_attr( $id ); ?>" class="<?= esc_attr( $class_name ); ?>">
		<?php if ( $block_style === 'v1' ) : ?>
			<?= get_template_part( 'template-parts/blocks/section-contact/variants/section-contact-v1', '', ['block_id' => $block['id'], 'is_preview' => $is_preview] ); ?>
		<?php elseif ( $block_style === 'v2' ) : ?>
			<?= get_template_part( 'template-parts/blocks/section-contact/variants/section-contact-v2', '', ['block_id' => $block['id'], 'is_preview' => $is_preview] ); ?>
		<?php endif; ?>
	</section>
<?php endif; ?>